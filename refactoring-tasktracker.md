...
- Размер классов не превышает 300 строк
...
- Единый формат возврата результатов валидации
...
### Этап 4: Улучшение структуры кода
...
**Описание**:
Вынести HTML код из PHP классов в отдельные view файлы для улучшения читаемости и поддержки.

**Шаги**:
1. Создать директорию `views/admin/`
2. Создать шаблоны для каждого раздела админки
3. Создать базовый класс View с методами рендеринга
4. Заменить inline HTML на вызовы view методов
5. Добавить поддержку локализации в шаблонах

**Структура**:
```
views/admin/
├── layout.php              # Основной layout
├── export-section.php      # Секция экспорта
├── import-section.php      # Секция импорта
├── history-section.php     # История миграций
├── schedule-section.php    # Расписание
└── user-preview.php        # Preview пользователей
```

**Класс для создания**:
- `Core\View\ViewRenderer` - базовый класс для рендеринга шаблонов

**Файлы для создания**:
- Все шаблоны в `views/admin/`
- `includes/Core/View/ViewRenderer.php`

**Критерии завершения**:
- Весь HTML вынесен в шаблоны
- PHP классы не содержат HTML
...
**Описание**:
Убрать лишние `require_once` из `mksddn-migrate-content-core.php`, полагаясь на autoloader.

**Шаги**:
1. Проверить, что все классы загружаются через autoloader
2. Удалить дублирующиеся `require_once` из core файла
3. Оставить только критичные загрузки (если есть)
...
**Критерии завершения**:
- Минимум `require_once` в core файле
...
**Описание**:
Создать Data Transfer Objects для типизированной передачи данных между слоями.

**DTO классы для создания**:
- `DataTransferObjects\ExportRequest` - запрос на экспорт
- `DataTransferObjects\ImportRequest` - запрос на импорт
- `DataTransferObjects\ExportResult` - результат экспорта
- `DataTransferObjects\ImportResult` - результат импорта
- `DataTransferObjects\FileUpload` - информация о загруженном файле
- `DataTransferObjects\ContentSelection` - выбор контента (улучшить существующий)

**Шаги**:
1. Создать директорию `includes/DataTransferObjects/`
2. Реализовать DTO классы с валидацией
3. Заменить массивы на DTO в методах
4. Обновить type hints

**Файлы для создания**:
- Все перечисленные DTO классы
