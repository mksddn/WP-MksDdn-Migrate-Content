...
class FullDatabaseImporter {
	...
	public function import( array $dump ) {
		...
		foreach ( $dump['tables'] as $table_name => $table_data ) {
			...
			$truncate = $wpdb->query( "TRUNCATE TABLE `{$table_name}`" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			if ( false === $truncate ) {
			...
			foreach ( $rows as $row ) {
				...
				$inserted = $wpdb->insert( $table_name, $row ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
				if ( false === $inserted ) {
					$wpdb->query( 'SET FOREIGN_KEY_CHECKS = 1' ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
	...
	private function ensure_table_exists( wpdb $wpdb, string $table_name, string $schema_sql ): void {
		...
		$wpdb->query( $schema_sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
	}
	...
	private function table_exists( wpdb $wpdb, string $table_name ): bool {
		...
		$found = $wpdb->get_var( $query ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
